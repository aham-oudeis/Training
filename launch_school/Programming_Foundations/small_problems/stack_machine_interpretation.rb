...
def minilang(str)
  input_commands = str.split.reverse.map {|item| item.to_i.to_s == item ? item.to_i : item }

  stack = []
  register = 0
  list_of_commands = %w(PUSH ADD SUB MULT DIV MOD POP PRINT)
  
  input_commands.reverse.each do |item|
    if list_of_commands.include?(item) == false
      register = item
    else
      case item
